...
export default function ChatWidget() {
    ...
    if (!session) return null; // Only logged in users can chat
        ...
        <div className="fixed bottom-6 right-6 z-50">
            {!isOpen ? (
                <button
                    ...
                    className="bg-[#D2691E] text-white p-4 rounded-full shadow-lg hover:bg-[#b05516] transition-all transform hover:scale-110 active:scale-95 flex items-center gap-2 group"
                >
                    <MessageCircle className="w-6 h-6" />
                    <span className="max-w-0 overflow-hidden group-hover:max-w-xs transition-all duration-500 font-sans text-sm font-bold uppercase tracking-wider">
                        Chat with Support
                    </span>
                </button>
            ) : (
                <div className="bg-white rounded-2xl shadow-2xl w-80 md:w-96 flex flex-col overflow-hidden border border-gray-100 animate-in slide-in-from-bottom-5 duration-300">
                    {/* Header */}
